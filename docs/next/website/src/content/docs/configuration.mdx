...
Control whether modified keys that have no legacy byte form (such as Shift+Enter or Ctrl+Tab) are forwarded to panes in their "extended" encoding (Kitty CSI u or xterm modifyOtherKeys):
...
`extended_keys = "auto"` honors the running program: the extended encoding is sent only when the program enabled the Kitty keyboard protocol or xterm modifyOtherKeys mode 2. Use `"off"` to never send it — modified keys fall back to their legacy byte (Shift+Enter becomes Enter), which avoids line-editor corruption in plain shells that don't understand these sequences. Use `"always"` to send the extended encoding even when the program never requested it. Keys that already have a legacy modified form (arrows, function keys, Shift+Tab, …) are unaffected.
