...
pane_history = false
# While prefix mode is active, temporarily switch the host input source to an
# ASCII-capable state so prefix commands register even when a CJK IME is
# active, then restore the previous state when prefix mode exits. macOS switches
# the system input source; Linux deactivates the active IME (fcitx5 or ibus).
# Best-effort; no-op on other platforms. Default: false.
