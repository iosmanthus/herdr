...
impl GhosttyPaneTerminal {
    ...
    /// enabling xterm modifyOtherKeys mode 2 (`CSI > 4 ; 2 m`). This reads the
    /// authoritative terminal state (the same state libghostty's encoder uses),
    /// not the caller-supplied protocol hint.
    ...
    pub fn encode_terminal_key_with_mode(
        ...
        // Force the Legacy protocol so `off` also overrides a Kitty pane.
...
mod tests {
    ...
    fn ghostty_plain_pane_encodes_shift_enter_as_cr() {
        ...
        // toggling the case of the last typed letter.
        let (tx, _rx) = mpsc::channel(4);
        let terminal = crate::ghostty::Terminal::new(80, 24, 0).unwrap();
        let pane = GhosttyPaneTerminal::new(terminal, tx).unwrap();
    ...
    fn ghostty_modify_other_keys_mode_one_sends_cr_for_shift_enter() {
        ...
        // a bare CR. (libghostty leaves no detectable mode-1 state, making it
        // indistinguishable from the default; both correctly fall back to CR.)
        let (tx, _rx) = mpsc::channel(4);
        let terminal = crate::ghostty::Terminal::new(80, 24, 0).unwrap();
        let pane = GhosttyPaneTerminal::new(terminal, tx.clone()).unwrap();
        let pane_id = PaneId::from_raw(1);
