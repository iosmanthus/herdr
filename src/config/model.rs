...
pub struct ExperimentalConfig {
    ...
    /// Linux it deactivates the active IME (fcitx5 or ibus); on Windows it
    /// switches the IME to English (ASCII) input. Windows support is currently
    /// limited to the Korean IME; with an IME for any other language, the input
    /// source is left unchanged. macOS, Linux and Windows only; a no-op
